...
const access_token = "IGQVJVZAnhyekZA4X2QtV0Vvd0ctcmswSmlVTC1SQ1NoakJyQk5tUHBxOE83OFpJcmxBazltdmlPS3Vva3k4VEdrT2U5TzlGd0xBdXF6b3hoMnJWeGttVHdvZA2JCMWtaQlF5c1F4ZA3l3ell0aFZArcHFVRwZDZD";
...
class Home extends Component {
    ...
    constructor() {
        ...
        this.state = {
            ...
            imageClicked: false,
            numLikes: 0,
            fixedCaption: "PG Diploma in Full Stack Development",
            searchbox: ""
        }
    }
    ...
    callBackFunction = (searchBoxData) => {
        this.setState({searchbox: searchBoxData});
        var that = this;
        var temp = this.state.allImagesData.filter(function(movie) {
            return that.state.fixedCaption.toLowerCase().includes(searchBoxData.toLowerCase())
        })
        that.setState({allImagesData: temp});
    ...
    render() {
        ...
        return (
            <div>
                <Header loginSuccess={true} profilepicture={profile_picture} parentCallBack={this.callBackFunction} />
                    ...
                    {this.state.allImagesData.map(movie => (
                        <Card className={classes.Card} key={"movie" + movie.id}>
                            ...
                            <CardContent>
                                ...
                                <span>
                                    <FavoriteIcon id={movie.id} className="red" onClick={this.likeHandler} />
